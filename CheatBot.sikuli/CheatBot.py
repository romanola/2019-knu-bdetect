...
def EatApple(delay):
        try:
            click(Pattern("1548242462659.png").similar(0.85))
...
def EatMushroom(delay):
        try:
            click(Pattern("1548242547155.png").similar(0.85))
        except: 
            EatApple(delay)
            return
        x, y = GetMouseCoordinates()
...
def RunNW(delay):
    ...
    if exists(Pattern("1548085434717.png").similar(0.80).targetOffset(-205,339)):
        click(Pattern("1548085434717.png").similar(0.60).targetOffset(-205,339))
        x, y = GetMouseCoordinates()
        wait(GetDelay(x, y))
    else: 
        popup(EJIK_NOT_FOUND)
    ...
    if res == 'apple':
        ...
        if go !=  None:
            ...
            EatApple(1)
             ...
             go = exists(Pattern("1548242547155.png").similar(0.85), 1)  
             if go !=  None:
